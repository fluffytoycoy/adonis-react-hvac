...
class ReviewSlider extends Component {
  ...
  componentWillMount(){
    //set mount state to true and fetch reviews
      this.setState({
        _isMounted: true
      }, ()=>{
        this.getReviews();
  ...
  getReviews() {
    //temp reviews from google api until server route is set up
    const reviews = [
      {
        "author_name": "Mike A",
      ...
      setTimeout(()=>{
        //if comp is still mounted after callback add reviews otherwise do nothing
  ...
  componentWillUnmount(){
    //mounted state must be set this way on unmount
    //cancels out all pending async functions
